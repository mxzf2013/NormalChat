...
public class LocalUDPSocketProvider {
	...
	private static final String TAG = LocalUDPSocketProvider.class
			.getSimpleName();
	...
	public void initSocket() {
		try {
			...
			this.localUDPSocket = new DatagramSocket(Configs.localUDPPort);
			...
			this.localUDPSocket.connect(
					InetAddress.getByName(Configs.serverIP),
					Configs.serverUDPPort);
			this.localUDPSocket.setReuseAddress(true);
			System.out.println(TAG + ":new DatagramSocket()已成功完成.");
		} catch (Exception e) {
			System.err.println(TAG + ":localUDPSocket创建时出错，原因是：" + e.getMessage());
		}
	}
